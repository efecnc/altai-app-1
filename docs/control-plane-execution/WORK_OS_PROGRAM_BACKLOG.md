...
| 101 | Per-workspace single-writer cutover | in_progress | 053, 060, 100 | #80, #82, #84, #86, 1–2 | No workspace has two authoritative mutation paths |
...
1. `CP-08-109` — assignments/todos import cutover (101 slice C.b)
   (Wire the dormant CP-08-104 legacy importer behind the feature-flag
   ledger: the import command refuses mechanically while any session
   holds the workspace assignment store (slice-A lock + a mechanical
   session-activity check), DEC-008's failed→in_progress+phase-failed
   status translation applies at the flip, and after import new-work
   writes go to canonical rows while the legacy JSON stores become
   read-history for old sessions — never dual-write, never silent loss.
   The M1 `work_items` UI store stays behind the LegacyWorkBridge;
   orchestration intents and issue/pr ExternalObject linking remain
   scoped out; no legacy deletion (package 102); C.c recovery selection
